...
export default function ReviewPage() {
  ...
  const handleRevision = async () => {
    ...
    try {
      await updateDoc(doc(db, "projects", id), {
        status: "production",
        clientFeedback: revisionText,
      });
      alert("Revision requested! Our team will get back to work.");
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-6">
            ...
            <div className="flex items-center gap-2 border-b-2 border-indigo-600 pb-1">
              <span className="bg-indigo-600 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full font-bold shadow-sm">
                3
              </span>
            ...
            <div className="flex items-center gap-2 opacity-50">
              <span className="bg-gray-200 dark:bg-gray-800 text-gray-500 text-[10px] w-5 h-5 flex items-center justify-center rounded-full font-bold">
                4
              </span>
          ...
          <div className="max-w-5xl mx-auto w-full space-y-6">
            ...
            <div className="relative group aspect-video bg-black rounded-xl overflow-hidden shadow-2xl border border-gray-200 dark:border-gray-800 flex items-center justify-center">
              {projectData?.reviewUrl ? (
                <iframe
                  src={projectData.reviewUrl}
                  className="absolute inset-0 w-full h-full border-0"
                  allowFullScreen
                  title="Project Review Video"
                ></iframe>
              ) : (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-900 text-white p-8 text-center bg-gradient-to-br from-indigo-900/40 to-purple-900/40 opacity-80">
                  <Clapperboard className="w-12 h-12 text-indigo-400 mb-4 opacity-75" />
                  <h3 className="text-lg font-bold mb-2 tracking-tight">
                    Your video is currently in the editing bay.
                  </h3>
                  <p className="text-sm text-gray-400 max-w-md mx-auto leading-relaxed">
                    Our AI agents and human directors are assembling the master
                    cut based on your blueprint. Check back soon.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4">
              <div className="w-full sm:w-auto relative">
                {!showRevisionInput ? (
                  <button
                    ...
                    className="w-full sm:w-auto flex items-center justify-center gap-2 text-gray-600 dark:text-gray-300 font-semibold px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-700 hover:bg-white dark:hover:bg-gray-800 transition-all"
                  >
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-[400px]">
                    <textarea
                      ...
                      placeholder="Detail your revisions here..."
                      className="w-full p-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg text-sm text-slate-900 dark:text-white outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all resize-none h-12"
                    />
                    <button
                      onClick={handleRevision}
                      disabled={isRevising || !revisionText.trim()}
                      className="bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-bold px-4 py-2.5 rounded-lg disabled:opacity-50 whitespace-nowrap h-12"
                    >
                      {isRevising ? "Sending..." : "Submit"}
                    </button>
                    <button
              ...
              <button
                ...
                disabled={isApproving || !projectData?.reviewUrl}
                className="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-8 py-3 rounded-lg shadow-lg shadow-indigo-600/20 flex items-center justify-center gap-2 transition-transform active:scale-95 disabled:opacity-50"
              >
        ...
        <aside className="w-full lg:w-[35%] bg-gray-50 dark:bg-gray-900/30 flex flex-col h-[50vh] lg:h-full border-t lg:border-t-0 border-gray-200 dark:border-gray-800 shrink-0">
          ...
          <div className="p-6 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between bg-white dark:bg-gray-900 shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 text-xs px-2.5 py-0.5 rounded-full font-bold">
                3
              </span>
            </div>
            <button
            ...
            {[
              {
                name: "Sarah Chen",
                time: "0:04",
                text: "Can we make the UI zoom slightly faster here to match the beat drop?",
              },
              {
                name: "Marcus Thorne",
                time: "0:15",
                text: "Love this transition. The AI voiceover sounds incredibly natural.",
              },
              {
                name: "Sarah Chen",
                time: "0:22",
                text: "Make sure to emphasize the 14-day free trial text in this motion graphic.",
              },
            ].map((comment, idx) => (
